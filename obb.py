...
# 尝试导入numpy，优先使用本地版本，如果失败则使用系统版本
try:
    # 添加本地numpy路径
    current_dir = os.path.dirname(os.path.abspath(__file__))
    numpy_path = os.path.join(current_dir, 'python_libs')
    ...
    # 如果本地numpy导入失败，尝试使用系统numpy
    try:
        # 移除本地路径，使用系统numpy
        if numpy_path in sys.path:
            sys.path.remove(numpy_path)
        from numpy import ndarray, array, asarray, dot, cross, cov, array, finfo, min as npmin, max as npmax
        from numpy.linalg import eigh, norm
    except ImportError as e:
        raise ImportError(f"无法导入numpy库。请确保已安装numpy: {e}")
